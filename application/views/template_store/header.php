					...
					<div class="col-lg-7 col-11 col-sm-6">
						<form action="#" class="search">
							<div class="input-group">
								...
								<select class="js-example-basic-single form-control" name="kec" id="keclist">
									<option value="">Cari store berdaserkan kecamatan</option>
									<?php 
									foreach ($kec as $keclist) {
										$kab = $this->db->get_where('tbl_kabupaten', ['id' => $keclist['regency_id']])->row_array();
										?>
										<option value="<?= $keclist['id'] ?>">
											<?= ucfirst(strtolower($keclist['name'])).", ". ucfirst(strtolower($kab['name'])) ?>
										</option>
									<?php } ?>
								</select>
								<!-- <input type="text" class="form-control" placeholder="Cari berdasarkan kecamatan atau kabupaten"> -->
					...
					<div class="col-lg-2 col-1 col-sm-6">
						<div class="right-icons pull-right  d-lg-block">
							<div class="single-icon wishlist">
								...
								<?php 
								$order = $this->db->get_where('tbl_checkout',['kode_user' => $this->session->kode_user])->num_rows();
								?>
								<span class="badge badge-default"><?= $order ?></span>
							</div>
